...
public class Test {
	...
	protected static void testParse() {
		String str = "本产品不支持保单贷款。上海虹桥机场南路。" ;
		// 基本分词
		Result result = BaseAnalysis.parse(str);
		System.out.println(result);
		// 标准分词
		result = ToAnalysis.parse(str);
		System.out.println(result);
		// 自然语言分词
		result = NlpAnalysis.parse(str);
		System.out.println(result);
		// 用于检索的分词方式
		result = IndexAnalysis.parse(str);
		System.out.println(result);
	}
	...
	protected static void test2() {
		 ...
		 KeyWordComputer<NlpAnalysis> kwc = new KeyWordComputer<NlpAnalysis>(5);
